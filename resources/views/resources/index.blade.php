<x-app-layout>
    <header style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <div>
            ...
            <p style="color: #64748b; font-size: 14px;">Total managed assets: <strong>{{ $totalCount }}</strong></p>
        </div>
        ...
        <div style="display: flex; gap: 10px;">
            @auth
                ...
                @endif
            @else
                <a href="{{ route('login') }}" style="background: #1e293b; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">Sign In to Manage</a>
            @endauth
        </div>
    </header>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px;">
        ...
        <div style="background: white; border-radius: 15px; border: 1px solid #e2e8f0; padding: 20px; position: relative; transition: 0.3s;">
            ...
            <div style="border-top: 1px solid #f1f5f9; padding-top: 15px; display: flex; flex-wrap: wrap; gap: 10px;">
                @auth
                ...
                @else
                    {{-- Actions for Guests --}}
                    <span style="color: #94a3b8; font-size: 12px; font-style: italic;">Login to take action</span>
                @endauth
            </div>
        </div>
        @endforeach
